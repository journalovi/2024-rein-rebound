...
This is a fantastic way to showcase potential users the ability of a software package.
...
There are some drawbacks to using WebAssembly. 
...
And third, using WebAssembly can impact accessability. 
For example text rendered via WebAssembly and WebGL might not be picket up by operating system or browser accessability tools such as screenreaders.
For this reason, we do not render text via WebGL in our implementation and instead use HTML for this.

To measure the performance of the three different operating modes, we run the self-gravitating disk simulation from @fig-livesim with $N=10^4$ as well as $N=10^5$ particles and measure the average frames per second of the visualizations.
...
Without the simulation paused, the visualization performs significiantly better (29fps). 
...
First, we can reuse our existing C code that uses OpenGL for rendering, with only minimal changes related to the rendering loop and the rendering of on-screen text.^[Rendering text is non-trivial in pure OpenGL. However, it is very easy in a browser with the additional benefit of better supporting accessibility tools such as screen readers.]
...
Since we are implementing both the front and backend, we have tight control over what kind of requests the server needs to be able to respond to.
...
Just as for the other operating modes, this workflow does not rely on any external libraries, neither on the C nor the IPython side. 
...
The most recent development version is available on [GitHub](https://github.com/hannorein/rebound). 
The version of REBOUND that was used for this paper is also available as an archive on [Zenodo](https://zenodo.org/records/15784767).
