...
Second, debugging WebAssembly code might pose a challenge given that the compiled code makes it harder to inspect the code in the browser and due to a lack of debugging tools.
And third, using WebAssembly can impact accessibility. 
For example text rendered via WebGL might not be picket up by operating system or browser accessibility tools such as screen readers (the same is true for OpenGL).
...
- We disable the OpenGL based rendering of on-screen text. Rendering text is non-trivial in pure OpenGL[^simplefont]. However, it is very easy in a browser with the additional benefit of better supporting accessibility tools such as screen readers. Rendering text can be done by updating an HTML elemen by [calling javascript code from our C](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L135) with emscripten's ``EM_JS`` API.
- We also need to adjust the rendering loop. On the desktop version the rendering loop is using ``glfwGetTime()`` to call the rendering function at a [fixed frame rate](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L1718). On the emscripten side, we use the ``emscripten_request_animation_frame_loop()`` API call instead. 

After making the above changes, one should be able to compile the code with emscripten and run simulation together with the visualization in the browser (i.e. what we call the browser mode). This is a good opportunity to fix any new bugs or rendering issues that arose from the recent changes.

- To get the hybrid mode working next, we need to implement a web server. The [reference implementation](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c) in REBOUND can be used as a starting point. The two required capabilities of the web server are to [serve a static HTML file](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L384) (running the browser based visualization) and [send updated simulation data](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L320) on request. 
- The web server gets started on a separate thread when a simulation is created. It initially [checks](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L215) if it can find the static html file the browser needs and, as a fallback, downloads it from the internet if does not exist. The latter is not strictly necessary but allows users to avoid downloading the file manually.
- What simulation data to send to the browser at every frame depends on the nature of the simulation. In REBOUND, we use the Simulationarchive to serialize the entire simulation. This way, we don't need to implement complicated logic for changing simulation states as the Simulationarchive contains all information to reconstruct the simulation. Alternatively, we could only send the updated particle data, but then we would need to handle special cases, for example when the particle number increases or decreases which involves reallocating various arrays. For other simulations, e.g. a hydrodynamic simulation with a fixed size grid, the latter option would achieve better performance.
...
At this point, all the components in order for the hybrid mode to work are in place. The next steps are to polish the [HTML file](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/web_client/shell_rebound_webgl.html) that emscripten uses as a template so that it can show some status information, render any onscreen text, and if desired send user interactions back to the server. 
