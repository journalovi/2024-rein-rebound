...
The [REBOUND documentation](https://rebound.readthedocs.io/en/latest/c_examples/selfgravity_disc/) makes extensive use of the browser mode, offering users the ability to run all examples directly in the browser and thus showcase the ability of the software package.
...
First, we can reuse our existing C code that already uses OpenGL for rendering, with only minimal changes (see below).
...
WebAssembly compilers such as emscripten are still relatively new and are not yet as widely available and as well tested as C compilers.
Second, debgugging WebAssembly code might pose a challenge given that the compiled code makes it harder to inspect the code in the browser and due to a lack of debugging tools.
And third, using WebAssembly can impact accessibility. 
For example text rendered via WebGL might not be picket up by operating system or browser accessibility tools such as screenreaders (the same is true for OpenGL).
For this reason, we do not render text via WebGL in our implementation and instead use HTML for this.


## Performance 
To measure and compare the performance of the three different operating modes, we run the self-gravitating disk simulation from @fig-livesim with $N=10^4$ as well as $N=10^5$ particles and measure the average frames per second we achieve with each visualization mode.
The tests were run on a 2020 MacBook Air (M1). 
Safari was used as a browser but the measurements were almost idential for Chrome and Firefox.
The results are shown in @tbl-performance. 

For small simulations ($N=10^5$), the performance is comparable for all three modes (42 - 71fps). 
The hybrid mode is the slowest, which makes sense given the additional data transfer over http.
...
This can by verified by pausing the simulation, in which case the visualization performs significiantly better (29fps). 
WebAssembly and emscripten support multithreading and shared memory access across threads (SharedArrayBuffer).
...
[^COOP]: For more details on Cross Origin Opener Policy (COOP) and Cross Origin Embedder Policy (COEP) headers see [the emscripten documentation](https://emscripten.org/docs/porting/pthreads.html).

| Mode | $N=10^4$ particles | $N=10^5$ particles |
|---------|:-----|:-----|
| OpenGL   |  71 fps |  41 fps    |
| Hybrid   |  42 fps |  2.2 fps   |
| Browser  |  54 fps |  3.7 fps   |
...
For the browser mode, one can simply use static files which the browser can access, or even include the data within a single HTML file (see @fig-livesim).
...
To further facilitate the rendering of complex animations, we also implement a way for the server side to push the view matrix and other visualization setting to the browser and override any client side user interaction. 
...
## Jupyter Notebooks
Although REBOUND is written in C, it also comes with a python interface, allowing users to more easily setup and run N-body simulations that do not require modifying the REBOUND source code itself.
...
Because this is using the hybrid mode, this workflow does also not rely on any external libraries, neither on the C nor the IPython side. 


## Step by step guide 

Here, we summarize the adjustments we had to make to the existing OpenGL code in REBOUND in order to add the new hybrid and browser modes.
...
- The OpenGL shaders in the browser use an Embedded Systems (ES) version. [This](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L1032) [difference](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L1087) is hard coded with a precompiler directive.
- Because the capabilities of the ES version are more restricted, some features have been disabled with a precompiler directive. For example, we disable [multi-sampling](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L1075).
- We disable the OpenGL based rendering of on-screen text. Rendering text is non-trivial in pure OpenGL[^simplefont]. However, it is very easy in a browser with the additional benefit of better supporting accessibility tools such as screen readers. All we need to do is update an HTML element which we can do by [calling javascript from our C code](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L135) with emscripten's ``EM_JS`` API.
- We also need to adjust the rendering loop. On the desktop version the rendering loop is using ``glfwGetTime()`` to call the rendering function at a [fixed framerate](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/display.c#L1718). On the emscripten side, we use the ``emscripten_request_animation_frame_loop()`` API call instead. 

After making the above changes, one should be able to compile the code with emscripten and run the visualization in the browser (i.e. what we call the browser mode). This is a good oportunity to fix any new bugs or rendering issues.

- To get the hybrid mode working next, we need to implement a webserver. The [reference implementation](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c) in REBOUND can be used as a starting point. The two required features are that the server can [serve a static HTML file](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L384) (running the browser based visualization) and [send updated simulation data](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L320) on request. 
- The webserver gets started on a separate thread when a simulation is created. It initially [checks](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/server.c#L215) if it has the static html file the browser needs and downloads it from the internet if not.
- What simulation data to send depends on the nature of the simulation. In REBOUND, we use the Simulationarchive to serialize the entire simulation. This way, we don't need to implement complicated logic for changing simulation states as the Simulationarchive contains all information to reconstruct the simulation. Alternatively, we could only send the updated particle data, but then we would need to handle special cases, for example when the particle increases or decreases. But for other simulations, e.g. a hydrodynamic simulation with a fixed grid, the later option would achieve better performance.
- While the simulation is getting updated during a timestep, it [locks a Mutex](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/src/rebound.c#L850) such that the server thread doesn't send a partially updated simulation with potentially corrupted memory to the browser.
- On the browser side, we [create a new simulation](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/web_client/problem.c#L166) and visualize it the same way as we do for the browser mode. However, instead of initializing the simulation with particle data locally, [we request](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/web_client/problem.c#L150) a Simulationarchive from the server, then use that to update the in-browser simulation which in turn updates the visualiztion. Once done, we [pause](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/web_client/problem.c#L90) for a few milliseconds before requesting another Simulationarchive). 

At this point, all the components for the hybrid mode to work are in place. The next steps are to polish the [HTML file](https://github.com/hannorein/rebound/blob/4a0bdb056f684c397a10598d270f52df3bce883c/web_client/shell_rebound_webgl.html) that emscripten uses as a template so that it can show some status information and render any onscreen text. 
